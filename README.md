...
| 色数 | 6 | 糸の本数の**上限**。実際の色数は色の統合により自動で決まる |
| 色の統合 | 標準 | 似た色をどこまで1本の糸にまとめるか (弱/標準/強)。色が多すぎるときは「強」に |
...
1. **減色** (`src/digitize/quantize.ts`) — 透明部と画像端の均一色を背景として除去し、**知覚色空間 Oklab** の k-means で指定色数に減色。アンチエイリアス (輪郭のぼかし) の中間色がクラスタにならないよう遷移画素をクラスタ推定から除外し、知覚的に近い色・微小な色は1本の糸に統合する (面積が小さくても黒い線画のような独立した色は残す)。モードフィルタと小領域マージでノイズを除去
2. **輪郭抽出** (`src/digitize/contour.ts`) — 色ごとに領域の輪郭ループ (外周+穴) を抽出し、Douglas-Peucker で簡略化 → 角保持つき Chaikin 平滑化でピクセル境界のガタガタを除去
3. **ステッチ生成** (`src/digitize/fill.ts`, `skeleton.ts`, `outline.ts`) — コンポーネント (同色の連結領域) ごとに縫い方を自動選択:
   - 広い領域 → 走査線方式のタタミ縫い (レンガ状の針落ち、穴をまたぐ移動は渡り糸に変換)
   - 細い線 (〜6mm) → **スケルトン (中心線) ベースのサテン縫い**。Ink/Stitch の auto-route と同じ方式で、細線化 → グラフ化 → 「行き=アンダーパス、帰り=サテン」の一筆書きルートを生成。分岐やリングを含む連結した線ネットワークは糸切りゼロで縫い切る。サテンは距離変換による局所幅・局所方向のジグザグで曲線に追従
   - 極細の線 (〜1.5mm) → センターライン縫い (中心線に沿ったランニング)
4. **縫い順とつなぎ** (`src/digitize/pipeline.ts`) — 実機で糸切りが多発しないことを最優先:
   - 同色内は nearest-neighbor で最短移動順に並べ替え (開いた run は反転、閉ループは最寄り点から縫うよう回転)
   - **「隠れる経路」を通る移動だけ**を糸を切らずつなぎ縫いで接続: 同色領域の内側、または自分より後に縫われる色の上 (後の縫いで覆われる)。背景やすでに縫った色を横切る移動は糸切り+ジャンプにし、渡り糸が見える問題を防ぐ
   - 糸切り (TRIM) は「離れた島への移動」と「色替え前」のみ。面や連結した線の途中では絶対に切らない
