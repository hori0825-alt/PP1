// PES (PEC) / DST リーダー。出力したファイルを読み戻して検証するために使う。
// エンコーダー (pes.ts/dst.ts) と対になり、ラウンドトリップ検証・将来の
// インポート機能の土台になる。
...
/** DST レコードを内部座標系 (中心原点・+y下) の絶対ステッチ点に復元する */
export function dstStitchPoints(dec: DecodedDst): Point[] {
  let x = 0;
  let y = 0;
  const pts: Point[] = [];
  for (const r of dec.records) {
    x += r.dx;
    y += r.dy;
    if (r.kind === "stitch") pts.push({ x, y: -y }); // DST は Y 上向きなので反転
  }
  return pts;
}
...
export function decodePes(data: Uint8Array): DecodedPes {
  ...
  const label = new TextDecoder("ascii").decode(data.slice(pecOffset + 3, pecOffset + 19)).trimEnd();
...
/** PEC オペを内部座標系の絶対ステッチ点に復元する (通常ステッチのみ) */
export function pesStitchPoints(dec: DecodedPes): Point[] {
  let x = 0;
  let y = 0;
  const pts: Point[] = [];
  for (const op of dec.ops) {
    x += op.dx;
    y += op.dy;
    if (op.kind === "stitch") pts.push({ x, y });
  }
  return pts;
}
