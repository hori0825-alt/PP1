...
```
src/report/qr.ts        … 最小 QR エンコーダ (バイトモード/レベルM/V1-3) +
                          デコーダ (ラウンドトリップ検証用) + qrToSvg。依存なし
src/report/workorder.ts … buildWorkOrder: 使用糸一覧/統計/推奨設定をまとめる (純粋)
src/library/store.ts    … DesignLibrary (KeyValueStore 注入式) + MemoryStore。
                          保存/検索/タグ/お気に入り/最近使った
src/ui/report.ts        … 作業指示書を HTML 化し印刷ウィンドウで PDF 保存。
                          QR/プレビューの SVG ダウンロード
src/ui/library.ts       … localStorage 注入、ライブラリタブ UI、現デザイン保存/読込
src/ui/main.ts          … 出力タブに作業指示書/QR/プレビュー、ライブラリタブ追加
test/qr.test.ts (11) / test/report.test.ts (8)
```

#### QR コード (実際にスキャン可能)

- GF(256)/Reed-Solomon/BCH フォーマット情報/8マスク+ペナルティ選択を自前実装
- バイトモード・レベル M・バージョン1〜3 (単一ブロック、〜42バイト)
- 正しさは encode→decode ラウンドトリップで担保 (PES/DST と同流儀)。
  "PP1-LX9F2-A3" → V1 21×21 → デコードで元文字列一致を確認

#### 作業指示書 (PDF)

- 依存を増やさず、HTML 作業指示書を印刷ウィンドウで開き「PDF に保存」。
  使用糸一覧 (Brother色番号・縫い順・針数)・総針数・色替え・糸切り・
  サイズ・推定時間・推奨糸針 (布地レシピ由来)・QR・プレビュー・注意を掲載
- QR とプレビューは SVG 単体でも保存可能

#### デザインライブラリ

- localStorage 永続化。サムネイル (SVG プレビュー)・名前・サイズ・色数・針数・
  使用糸・タグ・お気に入り・メモ・プロジェクト JSON 紐付け
- 検索 (名前/タグ/メモ)・タグ AND フィルタ・お気に入りのみ・最近使った
- 「現在のデザインを保存」「開く」(プロジェクト復元)「削除」

#### 検証状況 (計190テスト)

- QR: 5種の文字列ラウンドトリップ・バージョン選択・容量超過例外・
  ファインダ/タイミング/ダークモジュール構造・SVG 生成
- workorder: 使用糸一覧/Brother色割当/針数合計=総針数/推奨設定
- library: 保存更新削除/検索/タグAND/お気に入り/最近使った/永続化/タグ列挙
...
- QR はバイトモード・レベル M・V1〜3 (〜42バイト) のみ。数値/英数モードの
  最適化や V4+ (複数ブロック) は未対応。誤り訂正は生成のみ (デコーダは検証用で
  訂正なし)。作業指示書 PDF はブラウザ印刷 (ネイティブ PDF 生成ライブラリ不使用)
- ライブラリは localStorage (ブラウザ単体)。クラウド/WiFi 同期は将来
